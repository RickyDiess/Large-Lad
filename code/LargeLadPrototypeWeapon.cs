...
public sealed class LargeLadPrototypeWeapon : Component
{
	...
	protected override void OnUpdate()
	{
		...
		// Send where the owner aimed, not the locally resolved object. Hammer
		// geometry is loaded independently on every machine, so its GameObject is
		// intentionally not a shared network identity. The host replays the shot
		// against its own authoritative scene from the player's eye position.
